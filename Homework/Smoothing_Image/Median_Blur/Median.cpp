...
int median_slider = 0;						// 設定中值濾波器滑動條的初始值

Mat src; Mat dst;
const char* file_name;
char window_name[] = "Median Demo";			// 設定視窗名稱
...
int noise( void )
{
	...
	// Applying Median blur  使用中值濾波器
	if( display_caption( "Median Blur (After)" ) != 0 ) { return 0; }
	for ( int i = 1; i < MAX_KERNEL_LENGTH; i = i + 2 )
	  { medianBlur ( src, dst, i );			// 中值濾波器函式
...
int trackbar( void )
{
	src = imread( file_name, 1 );	// 載入來源圖片
	dst = src.clone();
	createTrackbar( "Median blur", window_name, &median_slider, MAX_KERNEL_LENGTH, on_trackbar );
	on_trackbar( median_slider, 0 );
...
{
	medianBlur( src, dst, 2 * median_slider + 1 );	// 中值濾波器函式
	imshow( window_name, dst );
