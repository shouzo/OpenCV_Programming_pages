...
int gaussian_slider = 0;					// 設定gaussian濾波器滑動條的初始值

Mat src; Mat dst;
const char* file_name;
char window_name[] = "Gaussian Demo";			// 設定視窗名稱
...
int noise( void )
{
	...
	for ( int i = 1; i < MAX_KERNEL_LENGTH; i = i + 2 )
	  { GaussianBlur( src, dst, Size( i, i ), 0, 0 );	// Gaussian濾波器函式
...
int trackbar( void )
{
	src = imread( file_name, 1 );	// 載入來源圖片
	dst = src.clone();
	createTrackbar( "Gaussian blur", window_name, &gaussian_slider, MAX_KERNEL_LENGTH, on_trackbar );
	on_trackbar( gaussian_slider, 0 );
...
{
	GaussianBlur( src, dst, Size( 2 * gaussian_slider + 1, 2 * gaussian_slider + 1 ), 0, 0 );	// Gaussian濾波器函式
	imshow( window_name, dst );
