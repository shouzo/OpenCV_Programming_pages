...
int homogeneous_slider = 0;					// 設定Homogeneous濾波器滑動條的初始值

Mat src; Mat dst;
const char* file_name;
char window_name[] = "Homogeneous Demo";			// 設定視窗名稱
...
int noise( void )
{
	...
	// Applying Homogeneous blur  使用Homogeneous濾波器
	if( display_caption( "Homogeneous Blur (After)" ) != 0 ) { return 0; }
	for ( int i = 1; i < MAX_KERNEL_LENGTH; i = i + 2 )
	  { blur( src, dst, Size( i, i ), Point(-1,-1) );	// 平均濾波器函式
...
int trackbar( void )
{
	src = imread( file_name, 1 );	// 載入來源圖片
	dst = src.clone();
	createTrackbar( "Homogeneous blur", window_name, &homogeneous_slider, MAX_KERNEL_LENGTH, on_trackbar );
	on_trackbar( homogeneous_slider, 0 );
...
{
	blur( src, dst, Size( 2 * homogeneous_slider + 1, 2 * homogeneous_slider + 1 ), Point(-1,-1) );	// 平均濾波器函式
	imshow( window_name, dst );
