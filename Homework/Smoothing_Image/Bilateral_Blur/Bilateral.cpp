...
int bilateral_slider = 0;					// 設定雙邊濾波器滑動條的初始值

Mat src; Mat dst;
const char* file_name;
char window_name[] = "Bilateral Demo";		// 設定視窗名稱
...
int noise( void )
{
	...
	// Applying Bilateral blur  使用雙邊濾波器
	if( display_caption( "Bilateral Blur (After)" ) != 0 ) { return 0; }
	for ( int i = 1; i < MAX_KERNEL_LENGTH; i = i + 2 )
	  { bilateralFilter ( src, dst, i, i*2, i/2 );	// 雙邊濾波器函式
...
int trackbar( void )
{
	src = imread( file_name, 1 );	// 載入來源圖片
	dst = src.clone();
	createTrackbar( "Bilateral blur", window_name, &bilateral_slider, MAX_KERNEL_LENGTH, on_trackbar );
	on_trackbar( bilateral_slider, 0 );
...
{
	bilateralFilter ( src, dst, bilateral_slider, bilateral_slider * 2, bilateral_slider / 2 );	// 雙邊濾波器函式
	imshow( window_name, dst );
